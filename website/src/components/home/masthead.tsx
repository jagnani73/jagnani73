...
export const Masthead = () => {
  ...
  return (
    <section>
      <h1 className="m-0 overflow-hidden px-4 pt-[18px] pb-3 font-display text-[clamp(40px,13.5vw,60px)] leading-[0.95] tracking-[0.005em] text-tx rail:px-11 rail:pt-[26px] rail:pb-4 rail:text-[clamp(64px,8.6vw,126px)] rail:whitespace-nowrap">
        YASHVARDHAN JAGNANI
      </h1>
      <Rule />

      <div className="flex flex-wrap items-center gap-x-3.5 gap-y-2.5 overflow-hidden px-4 py-3 rail:flex-nowrap rail:gap-5 rail:px-11 rail:py-[15px] rail:whitespace-nowrap">
        <span className="font-display text-[clamp(26px,3.3vw,48px)] tracking-[0.02em] text-pri">
          BLOCKCHAIN
        </span>
        <span className="font-serif text-[clamp(22px,2.7vw,39px)] text-tx italic">
          {DECK}
        </span>
      </div>
      <Rule />
      ...
      <div className="grid grid-cols-1 font-mono text-[14px] leading-[1.8] text-tx2 rail:grid-cols-3">
        ...
        <div className="border-b border-rule px-4 py-3.5 rail:border-r rail:border-b-0 rail:px-11 rail:py-[18px]">
          ...
          <br />
          <span className="text-acc">{counts.RESEARCH}</span> IEEE papers ·
          105k+ npm downloads
        </div>
