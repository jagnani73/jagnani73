...
const STARS: [string, number, number, number][] = [
  ["Sirius", 6.7525, -16.716, -1.46], ["Canopus", 6.399, -52.696, -0.74],
  ["Arcturus", 14.261, 19.182, -0.05], ["Vega", 18.615, 38.784, 0.03],
  ["Capella", 5.278, 45.998, 0.08], ["Rigel", 5.242, -8.202, 0.13],
  ["Procyon", 7.655, 5.225, 0.34], ["Betelgeuse", 5.919, 7.407, 0.5],
  ["Achernar", 1.629, -57.237, 0.46], ["Hadar", 14.064, -60.373, 0.61],
  ["Altair", 19.846, 8.868, 0.77], ["Acrux", 12.443, -63.099, 0.77],
  ["Aldebaran", 4.599, 16.509, 0.85], ["Antares", 16.49, -26.432, 1.09],
  ["Spica", 13.42, -11.161, 1.04], ["Pollux", 7.755, 28.026, 1.14],
  ["Fomalhaut", 22.961, -29.622, 1.16], ["Deneb", 20.69, 45.28, 1.25],
  ["Mimosa", 12.795, -59.689, 1.25], ["Regulus", 10.139, 11.967, 1.35],
  ["Adhara", 6.977, -28.972, 1.5], ["Castor", 7.577, 31.888, 1.58],
  ["Gacrux", 12.519, -57.113, 1.59], ["Shaula", 17.56, -37.104, 1.62],
  ["Bellatrix", 5.418, 6.35, 1.64], ["Elnath", 5.438, 28.608, 1.65],
  ["Miaplacidus", 9.22, -69.717, 1.69], ["Alnilam", 5.604, -1.202, 1.69],
  ["Alnitak", 5.679, -1.943, 1.74], ["Alioth", 12.9, 55.96, 1.76],
  ["Dubhe", 11.062, 61.751, 1.79], ["Mirfak", 3.405, 49.861, 1.79],
  ["Wezen", 7.14, -26.393, 1.83], ["Alkaid", 13.792, 49.313, 1.85],
  ["Avior", 8.375, -59.51, 1.86], ["Menkalinan", 5.992, 44.947, 1.9],
  ["Atria", 16.811, -69.028, 1.91], ["Alphard", 9.46, -8.659, 1.98],
  ["Polaris", 2.53, 89.264, 1.98], ["Mirzam", 6.378, -17.956, 1.98],
  ["Hamal", 2.119, 23.462, 2.0], ["Diphda", 0.726, -17.987, 2.04],
  ["Nunki", 18.921, -26.297, 2.05], ["Mirach", 1.162, 35.621, 2.05],
  ["Alpheratz", 0.14, 29.09, 2.06], ["Kochab", 14.845, 74.156, 2.08],
  ["Saiph", 5.796, -9.67, 2.09], ["Denebola", 11.818, 14.572, 2.11],
  ["Algol", 3.136, 40.956, 2.12], ["Mintaka", 5.533, -0.299, 2.23],
  ["Mizar", 13.399, 54.925, 2.23], ["Schedar", 0.675, 56.537, 2.24],
  ["Caph", 0.153, 59.15, 2.28], ["Merak", 11.031, 56.383, 2.37],
  ["Scheat", 23.063, 28.083, 2.42], ["Phecda", 11.897, 53.695, 2.44],
  ["Markab", 23.079, 15.205, 2.49], ["Gamma Cas", 0.945, 60.717, 2.47],
  ["Enif", 21.736, 9.875, 2.4], ["Ruchbah", 1.43, 60.235, 2.68],
  ["Megrez", 12.257, 57.033, 3.31], ["Altais", 19.209, 67.661, 3.07],
];
const LINES: [string, string][] = [
  ...
  ["Caph", "Schedar"], ["Schedar", "Gamma Cas"], ["Gamma Cas", "Ruchbah"], ["Acrux", "Gacrux"],
  ["Dubhe", "Polaris"], ["Kochab", "Polaris"],
];
const LABELLED = new Set([
  "Sirius", "Vega", "Arcturus", "Capella", "Polaris", "Deneb", "Altair",
  "Betelgeuse", "Aldebaran", "Pollux", "Dubhe",
]);
...
const BG_STARS: [number, number, number][] = (() => {
  ...
  for (let i = 0; i < 320; i++)
    out.push([r() * 24, Math.asin(2 * r() - 1) / (Math.PI / 180), 2.8 + r() * 3]);
  return out;
})();
const HILL: number[] = (() => {
...
const TRAIL_N = 54;
const SPEEDS: [string, number][] = [["live", 0], ["730×", 730], ["7300×", 7300]];
const AZ0 = 0;
const ALT0 = 30;
...
export const SkyMap = () => {
  ...
  const drawAt = useCallback((vt: number) => {
    ...
    STARS.forEach(([, ra, dec, m]) => {
      ...
      for (let k = TRAIL_N; k >= 0; k--) {
        ...
        const lst = (((lstNow - (dtm * 360.98564736629) / 86400000) % 360) + 360) % 360;
    ...
    ([["N", 0], ["E", 90], ["S", 180], ["W", 270]] as [string, number][]).forEach(
      ([lab, az]) => {
  ...
  const useMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (p) => {
        setGeoMsg(null);
        setLoc({ lat: p.coords.latitude, lon: p.coords.longitude, name: "your location" });
      },
      (err) => {
        console.warn("[sky] geolocation failed", err);
        setGeoMsg(
          err.code === err.PERMISSION_DENIED ? "permission denied" : "unavailable",
        );
      },
    );
  };
